...
static bool domino_equal(domino tile1, domino tile2){
    bool equal = (domino_up(tile1) == domino_up(tile2)) && (domino_down(tile1) == domino_down(tile2));
    bool inverted = (domino_up(tile1) == domino_down(tile2)) && (domino_down(tile1) == domino_up(tile2));
...
{
    unsigned int res = 0u;
    line_iterator lit = line_iterator_begin(line);
    while (!line_iterator_end(lit)){
        if(domino_is_double(line_iterator_get(lit))){
            res++;
        }
...
bool is_in_line(domino_line line, domino tile)
{
    ...
    line_iterator lit = line_iterator_begin(line);
    while (!line_iterator_end(lit)){
        if(domino_equal(line_iterator_get(lit), tile)){
            return true;
        }
        lit = line_iterator_next(lit);
    }
